...
env_test = gym.make('robot-env-v0', config=CONFIG_TEST)
ENVS_LIST = [env_test]
...
def test_observation_space(env):
    ...
    assert obs_shape == (64, 64, 1) or (64, 64, 3) or (64, 64, 4)

@pytest.mark.parametrize("env", ENVS_LIST)
def test_action_spaces(env):
    assert env.action_space == gym.spaces.Box(-1, 1, shape=(4,))  \
                                or gym.spaces.Box(-1, 1, shape=(5,)) \
                                or gym.spaces.Box(-1, 1, shape=(7,)) 

@pytest.mark.parametrize("env", ENVS_LIST)
def test_reset_return(env):
    obs = env.reset()
    assert obs.shape == (64, 64, 1) or (64, 64, 3) or (64, 64, 4)
